    ...
    <div className="container mx-auto py-8 min-h-screen max-w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 p-4 px-20">
        {classes.map((classes, index) => (
          <Card key={index} className="relative bg-white shadow-sm hover:shadow-md transition-shadow duration-300 max-w-[600px] w-full mx-auto rounded-lg" >
            <CardContent className="p-0">
              <a href={`/sports-classes/${classes.sports_id}`} className="block">
                <div className="aspect-square relative rounded-lg overflow-hidden">
                  <img src={classes.image} alt={classes.title} className="object-cover w-full h-full rounded-lg" />
                </div>
                <div className="p-4">
              ...
              <a href={`/sports-classes/${classes.sports_id}`}>
                <Button size="sm" className="bg-[#ff2020] hover:bg-[#000000] text-white w-full text-sm py-2">
                  Learn More
                </Button>
              </a>
            </CardContent>
