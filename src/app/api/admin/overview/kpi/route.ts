...
export async function GET() {
  try {
    ...
    const todayRevenue = todayPayments.reduce(
      (sum, payment) => sum + payment.data.availedPlan.amount,
      0
    );
    ...
    const todayCustomers = new Set(
      todayPayments.map((p) => p.data.customer.customerId)
    ).size;
    const yesterdayCustomers = new Set(
      yesterdayPayments.map((p) => p.data.customer.customerId)
    ).size;
    ...
    const result = {
      ...
      transactions: todayPayments.map((payment) => ({
        ...
        customerType: payment.data.customer.type,
        planType: payment.data.availedPlan.name,
        timestamp: payment.data.createdAt,
        userId: payment.data.customer.customerId,
      })),
    };
